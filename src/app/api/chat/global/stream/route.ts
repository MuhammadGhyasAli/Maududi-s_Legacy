...
export async function POST(request: NextRequest) {
  ...
  const { systemInstruction, messages, language } = body;
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        // Build a search-optimized context from the system instruction
        // Since we don't have vector DB access from Next.js, we extract relevant
        // context by asking the LLM to filter the system instruction
        ...
        const formattedMessages = [
          ...
          ...(systemInstruction ? [{ role: 'system', content: `KNOWLEDGE BASE:\n${systemInstruction}` }] : []),
          ...messages.map((m: any) => ({

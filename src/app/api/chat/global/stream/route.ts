...
export async function POST(request: NextRequest) {
  ...
  const { systemInstruction, messages, language, conversationId } = body;
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        const backendRes = await fetch(`${BACKEND_URL}/chat/global`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ systemInstruction, messages, language, conversationId }),
        });

...
function FeatureCard({ feature }: { feature: (typeof featureCards)[0] }) {
  ...
  return (
    ...
    >
      <div className="h-full p-6 sm:p-7 rounded-2xl border border-gray-200 dark:border-gray-700/50 bg-gray-50 dark:bg-gray-800/60 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 flex flex-col">
        <div className="w-11 h-11 rounded-xl bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
...
function CategoryCard({ category, index }: { category: string; index: number }) {
  ...
  return (
    ...
    >
      <Link
        href={`/${slug}`}
        className="group relative block p-5 rounded-2xl border border-gray-200 dark:border-gray-700/50 bg-white dark:bg-gray-800 hover:border-emerald-400 dark:hover:border-emerald-600 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 overflow-hidden"
      >
        <div className="absolute inset-0 bg-gradient-to-br from-emerald-50/0 to-teal-50/0 group-hover:from-emerald-50/60 group-hover:to-teal-50/30 dark:group-hover:from-emerald-950/30 dark:group-hover:to-teal-950/20 transition-all duration-300" />
        <div className="relative">
          <div className="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-300">
            <Icon className="w-5 h-5 text-emerald-600 dark:text-emerald-400" aria-hidden="true" />
          </div>
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center">
        <span className="flex-shrink-0 w-10 h-10 rounded-xl bg-emerald-600 text-white flex items-center justify-center font-bold text-sm shadow-lg shadow-emerald-600/25">
          {step}
        </span>
        {index < 4 && (
          <div className="w-px flex-1 bg-emerald-200 dark:bg-emerald-800 mt-2" />
        )}
      </div>
...
export default function AboutPageClient() {
  ...
  return (
    <main className="min-h-screen">
        ...
        >
          {[
            ...
            <motion.div
              key={item.title}
              variants={itemVariants}
              className="group relative p-7 rounded-2xl border border-gray-200 dark:border-gray-700/50 bg-white dark:bg-gray-800/60 hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
            >
              <div className="w-12 h-12 rounded-xl bg-emerald-600 flex items-center justify-center mb-5 shadow-lg shadow-emerald-600/25 group-hover:scale-110 transition-transform duration-300">
      ...
      <section className="py-20 sm:py-28 bg-gray-50 dark:bg-gray-800/30 border-y border-gray-200 dark:border-gray-700/50">
        <div className="max-w-4xl mx-auto px-4">
          <motion.div
            ...
            transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] as const }}
          >
            <h2 className="font-display text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-8">
              Why This Platform{' '}
              <span className="bg-gradient-to-r from-emerald-600 to-teal-500 bg-clip-text text-transparent dark:from-emerald-400 dark:to-teal-300">
                Exists
              </span>
      ...
      <section className="py-20 sm:py-28 max-w-6xl mx-auto px-4">
        <div className="grid gap-16 lg:grid-cols-2 lg:items-start">
          ...
          <motion.div
            ...
            viewport={{ once: true, margin: '-80px' }}
          >
            <StepItem step={1} title="Explore Categories" description="Browse by topic: Tafsir, Politics, Theology, Economics, and more." index={0} />
            <StepItem step={2} title="Select a Book" description="View details, description, page count, and language availability." index={1} />
            <StepItem step={3} title="Read or Chat" description="Open the PDF reader, or click &ldquo;Chat with AI&rdquo; to ask questions." index={2} />
            <StepItem step={4} title="Search Everything" description="Use the AI Context Finder to search across the entire library." index={3} />
            <StepItem step={5} title="Upload & Identify" description="Use Image Search to identify book pages and find context instantly." index={4} />
          </motion.div>
        </div>
      </section>
            ...
            {techStack.map((item) => {
              ...
              return (
                <motion.div
                  key={item.name}
                  variants={itemVariants}
                  className="group p-5 rounded-2xl border border-gray-200 dark:border-gray-700/50 bg-white dark:bg-gray-800/60 hover:shadow-lg hover:border-emerald-300 dark:hover:border-emerald-700 transition-all duration-300"
                >

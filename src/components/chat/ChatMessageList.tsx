...
function SpeakerButton({ text }: { text: string }) {
  ...
  const { speak, stop, isSpeaking } = useSpeechSynthesis({
    onStart: () => { setIsPlaying(true); setError(null); },
    onEnd: () => setIsPlaying(false),
    onError: (err) => { setIsPlaying(false); setError(err); },
  });
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (isSpeaking) {
      ...
      speak(text).catch((err) => setError(err?.message || 'Playback failed'));
    }
  };
  ...
  return (
    <div className="relative inline-flex">
      <button
        onClick={handleClick}
        className={`${isPlaying || error ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} cursor-pointer p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 transition-all shrink-0`}
        title={isSpeaking ? 'Stop' : error ? 'Retry' : 'Read aloud'}
      >
        {isSpeaking ? (
          <svg className="w-3.5 h-3.5 text-brand-green" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <svg className={`w-3.5 h-3.5 ${error ? 'text-red-400' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M19.114 5.636a9 9 0 010 12.728M16.463 8.288a5.25 5.25 0 010 7.424M6.75 8.25l4.72-4.72a.75.75 0 011.28.53v15.88a.75.75 0 01-1.28.53l-4.72-4.72H4.51c-.88 0-1.704-.507-1.938-1.354A9.01 9.01 0 012.25 12c0-.83.112-1.633.322-2.396C2.806 8.756 3.63 8.25 4.51 8.25H6.75z" />
          </svg>
        )}
      </button>
      {error && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-2 py-1 bg-red-500 text-white text-[10px] rounded whitespace-nowrap shadow-lg z-50 pointer-events-none">
          {error}

...
export default function MainShell({ children }: { children: React.ReactNode }) {
  ...
  const isStandaloneRoute = isChatRoute || isAuthRoute;
  ...
  return (
    <ToastProvider>
      <div className="min-h-screen bg-brand-bg-light dark:bg-brand-bg-dark text-gray-900 dark:text-gray-100 transition-colors duration-300 flex overflow-x-clip">
        ...
        <div id="main-content" tabIndex={-1} className={`flex-1 flex flex-col min-w-0 ${isStandaloneRoute ? '' : (isDesktopSidebarCollapsed ? "lg:ml-16" : "lg:ml-64") + ' pt-20'}`}>
          <div className="flex-1 h-full">{children}</div>
          {!isStandaloneRoute && <Footer />}
        </div>

        {!isStandaloneRoute && <ScrollToTop />}
      </div>
    </ToastProvider>
  );
}
